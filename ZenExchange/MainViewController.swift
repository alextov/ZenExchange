...
class MainViewController: UIViewController {
    ...
    var dollarsPerBarrelComment: String!
    
    var currentTugrik: Tugrik = .UAH {
        willSet {
            switch newValue {
            case .UAH:
                tugrikName = "гривен"
            ...
            tugriksPerDollarComment = tugrikName + " за доллар"
            tugriksPerEuroComment = tugrikName + " за евро"
            dollarsPerBarrelComment = "долларов за баррель"
        }
    }
    ...
    func fetchQuotes() {
        ...
        NSURLConnection.sendAsynchronousRequest(request, queue: queue) { (response: NSURLResponse!, data: NSData!, error: NSError!) -> Void in
            ...
            let dollarQuote = parsedResults.valueForKey("USD\(self.currentTugrik.rawValue)=X") as? Double ?? 0.0
            let euroQuote = parsedResults.valueForKey("EUR\(self.currentTugrik.rawValue)=X") as? Double ?? 0.0
